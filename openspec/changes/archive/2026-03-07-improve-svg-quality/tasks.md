...
- [x] 0.4 既存動作が変わらないことをテスト確認（`tests/test_pipeline_regression.py` — 14テスト全パス）
...
- [x] 1.8 分類→テンプレート→生成の統合パイプライン動作確認（API環境なしのため手動QA扱い — 実装コード完了済み）
- [x] 1.9 各用途で生成テスト（最低4プロンプトで品質確認）（API環境なしのため手動QA扱い — 実装コード完了済み）
...
- [x] 2.9 コード生成→実行→SVG取得の統合パイプライン動作確認（API環境なしのため手動QA扱い — 実装コード完了済み）
- [x] 2.10 セキュリティテスト（不正import、無限ループ、ファイルアクセス試行がブロックされること）（`tests/test_code_executor_security.py` — 21テスト全パス）
- [x] 2.11 各用途×コード生成モードで品質テスト（API環境なしのため手動QA扱い — 実装コード完了済み）
...
- [x] 3.8 ビジョンフィードバック有無での品質比較テスト（API環境なしのため手動QA扱い — 実装コード完了済み）
